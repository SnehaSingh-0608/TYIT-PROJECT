...
const BlogPostAIAssistant = () => {
  ...
  //mutation
  const mutation = useMutation({ mutationFn: generateContentAPI, onSuccess: (data) => {
    console.log(data);
      ...
      <div className="bg-white rounded-xl shadow-xl overflow-hidden max-w-2xl w-full p-6">
        ...
        <div className="flex mt-5">
          <div className="mr-2 mb-2">
            <span className="test-sm font-semibold bg-green-200 px-4 py-2 rounded-full">
        ...
        {generatedContent && (
          <div className="mt-6 p-4 bg-gray-100 rounded-md">
            ...
            {mutation.isPending && (
              <Markdown>
                {`## Generating content for prompt: ${formik.values.prompt}`}
              </Markdown>
            )}
            {mutation.isSuccess && (
              <Markdown>{`${generatedContent.replace("Generated content for prompt: ```markdown", "")}`}</Markdown>
            )}
          </div>
        )}
      </div>
